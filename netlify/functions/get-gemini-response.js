...
exports.handler = async function (event) {
    ...
    try {
        ...
        if (modelIdentifier === 'HusonGen1') {
            ...
            // ★★★ 核心修正：建構一個符合 generateContent 期望的請求 ★★★
            const request = {
        ...
        if (error.status === 503 || (error.message && error.message.includes("overloaded"))) {
            return { statusCode: 503, body: JSON.stringify({ text: `哇，` + (modelIdentifier || '這個') + ` 模型現在太忙了，跟當機沒兩樣。你換個模型或晚點再試。` }) };
        }
        return { statusCode: 500, body: JSON.stringify({ text: "API 炸了，我也不知道什麼狀況。" }) };
    }
};